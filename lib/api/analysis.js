...
export const getPairedCohortAxisFinalURL = (taskUUID) => {
    if (!taskUUID) {
        return null;
    }

    return `${ANALYSIS_API_BASE}/paired_cohort_axis_final/?taskUUID=${encodeURIComponent(
        taskUUID
    )}`;
};
...
export const getHybridReferenceRunDemoURL = () => {
    return "/analysis/hybrid_reference/run_demo/";
};
