"use client";

import { useState } from "react";

import ExpCorrelationAnalysisView
    from "@/components/features/common/ExpCorrelation/ExpCorrelationAnalysisView";
import { useWorkflowExpCorrelationOptions }
    from "@/components/features/workspace/hooks/useWorkflowExpCorrelationOptions";
import { useWorkflowExpCorrelationPlotData }
    from "@/components/features/workspace/hooks/useWorkflowExpCorrelationPlotData";
import {
    getTaskData,
    isTaskSuccess,
} from "@/components/features/workspace/components/taskInformation/taskStatusUtils";

const WorkflowExpCorrelationSection = ({
    task,
    title = "Expression Correlation Plot",
    height = 620,
}) => {
    const taskType = task?.task_type;
    const taskData = getTaskData(task);

    const taskUUID = taskData.uuid;
    const isSuccess = isTaskSuccess(taskData.status);
    const isHybridReferenceTask = taskType === "HybridReferenceTask";

    const [queryConfig, setQueryConfig] = useState({
        type: null,
        gene1: null,
        gene2: null,
    });

    const {
        optionsData,
        validTypes,
        availableTypes,
        results,
        isLoading: isOptionsLoading,
        isError: isOptionsError,
    } = useWorkflowExpCorrelationOptions({
        taskType,
        taskUUID: isSuccess ? taskUUID : null,
    });

    const {
        plotData,
        titlePrimary,
        titleSecondary,
        isLoading: isPlotLoading,
        isError: isPlotError,
    } = useWorkflowExpCorrelationPlotData({
        taskType,
        taskUUID: isSuccess ? taskUUID : null,
        type: queryConfig.type,
        gene1: queryConfig.gene1,
        gene2: queryConfig.gene2,
    });

    return (
        <ExpCorrelationAnalysisView
            title={title}
            height={height}
            optionsData={optionsData}
            validTypes={validTypes}
            availableTypes={availableTypes}
            results={results}
            plotData={plotData}
            titlePrimary={titlePrimary}
            titleSecondary={titleSecondary}
            isOptionsLoading={isOptionsLoading}
            isOptionsError={isOptionsError}
            isPlotLoading={isPlotLoading}
            isPlotError={isPlotError}
            queryConfig={queryConfig}
            setQueryConfig={setQueryConfig}
            missingDescription={
                !taskUUID
                    ? "Missing task UUID"
                    : !taskType
                        ? "Missing task type"
                        : null
            }
            unavailableDescription={
                taskUUID && taskType && !isSuccess
                    ? "Expression correlation plot is available only after the task succeeds."
                    : null
            }
            emptyDescription="No expression correlation data"
            showTcgaBasedTag={isHybridReferenceTask}
            tcgaBasedTooltip="Expression values for this correlation plot are based on TCGA reference expression data."
        />
    );
};

export default WorkflowExpCorrelationSection;
